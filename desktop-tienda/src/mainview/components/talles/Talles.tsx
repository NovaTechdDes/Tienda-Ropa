...
import { TalleItem } from './TalleItem';

interface Props{
    talles: Talle[]
}

export const Talles = ({talles}: Props) => {
    
    const [talle, setTalle] = useState<string>('')
    const { crearTalle } = useMutateTalles();

    const handleSubmit = () => {
        if (!talle.trim()) return;
        crearTalle.mutateAsync({nombre: talle });
        setTalle('');
    }
  ...
  return (
    <div className='max-w-6xl mx-auto p-6 space-y-8 h-full bg-[var(--atelier-ink)]'>
        ...
        <div className='flex items-center gap-3 border-b border-[var(--atelier-border)] pb-6'>
            ...
            <div>
                <h1 className='text-3xl font-bold text-[var(--atelier-parchment)] tracking-tight'>Gestión de Talles</h1>
                <p className='text-[var(--atelier-parchment-muted)] text-sm mt-0.5'>Crea y administra los talles disponibles para tu tienda</p>
            </div>
        </div>
        ...
        <div className='bg-[var(--atelier-surface-1)] border border-[var(--atelier-border)] rounded-2xl p-6 shadow-xl transition-all hover:shadow-2xl hover:border-[var(--atelier-border-soft)]'>
            <div className='flex flex-col md:flex-row items-end gap-4'>
                <div className='flex-1 w-full space-y-2'>
                    <label htmlFor="talle" className='text-sm font-medium text-[var(--atelier-parchment-muted)] px-1 uppercase tracking-wider'>
                        Nombre del Talle
                    </label>
                    <input 
                        type="text" 
        ...
        <div className='space-y-4'>
            ...
            <div className='grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4'>
                {talles?.map((talle) => (
                    <TalleItem key={talle.id} talle={talle}/>
                ))}
            </div>
        </div>
    </div>
